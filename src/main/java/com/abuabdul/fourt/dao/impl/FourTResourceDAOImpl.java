...
public class FourTResourceDAOImpl extends FourTBaseDAO<Resource, Long>implements FourTResourceDAO {
	...
	public List<Resource> findResourceByName(String resourceName) throws FourTServiceException {
		TypedQuery<Resource> query = getEntityManager().createQuery("from Resource r where r.name=:resourceName", Resource.class);
	...
	public List<Resource> findResourceByTaskDate(Date taskDate) throws FourTServiceException {
		TypedQuery<Resource> query = getEntityManager().createQuery("from Resource r where r.taskDate=:taskDate", Resource.class);
	...
	public List<Resource> findResourceByTaskStatus(String taskStatus) throws FourTServiceException {
		TypedQuery<Resource> query = getEntityManager().createQuery("select r from Resource r join r.taskDetailList td where td.status=:taskStatus", Resource.class);
	...
	public List<Resource> findResourceByTaskDuration(Float taskDuration) throws FourTServiceException {
		TypedQuery<Resource> query = getEntityManager().createQuery("select r from Resource r join r.taskDetailList td where td.duration=:taskDuration", Resource.class);
